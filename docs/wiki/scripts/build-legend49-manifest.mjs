#!/usr/bin/env node
/**
 * legend49-atwiki-pages.json から fetch manifest を生成。
 * 用法: node build-legend49-manifest.mjs <batchIndex 0-based> [batchSize=20]
 * バッチ13〜（L1/L2 バッチ12の次）
 */
...
async function main() {
  ...
  const name = `manifest-batch${batchNum}-l49-${first}-${last}.json`;
