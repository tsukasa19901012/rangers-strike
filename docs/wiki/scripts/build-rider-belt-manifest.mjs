#!/usr/bin/env node
/**
 * rider-belt-atwiki-pages.json から fetch manifest を生成。
 * バッチ39〜（legend49 バッチ38の次）
 */
...
const PAGES = path.join(packageRoot, "src/rider-belt-atwiki-pages.json");
const BASE_BATCH = 39;
...
async function main() {
  ...
  const name = `manifest-batch${batchNum}-rb-${first}-${last}.json`;
