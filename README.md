...
| 操作位置 | 自軍ゾーン・手札は画面下側 |
| スクロール | 対戦中は **100dvh** のセッション内に収める（ページ全体はスクロールしない） |
| ゾーン内 | 手札・横長ゾーンは **ゾーン内の横スクロール** のみ |
| サイズ調整 | **CSS（メディアクエリ）を主**、不足時のみ横向き低高さで JS 縮小（`apps/web/lib/compactViewport.ts`） |
| ヘッダー等 | タイトル・フェイズガイドは **折りたたみ可能**（横向きでは初期状態で畳む） |

実装の参照: `apps/web/app/globals.css`（`.game--session`）、`apps/web/components/GameApp.tsx`
