#!/usr/bin/env node
/**
 * XGather / SX カード画像を grnrngr.com の xgather-series ページから取得。
 * XG1-001 は XG-001 形式の画像 URL に読み替える（シリーズ1のみ）。
 * SX-001 等は各シリーズページ末尾のシークレットレアから取得。
 *
 * Usage:
 *   node scripts/download-xgather-images.mjs [--concurrency N] [--dry-run]
 */
...
const SERIES_PAGES = [
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-1`,
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-2`,
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-3`,
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-4`,
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-5`,
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-6`,
  `${GRNRNGR_ORIGIN}/cards/rangers-strike/xgather-series-7`,
];
...
const manifestPath = path.join(packageRoot, "pipeline/data/xgather-image-download.json");
...
const XGATHER_CARD_ID_RE = /^(XG\d-\d{3}|SX-\d{3})$/;
...
/** grnrngr lightbox ID → リポジトリのカード ID（XG1-001 / SX-001 等）。 */
export function lightboxToCardId(lightbox) {
  const base = lightbox.replace(/\[.*\]$/, "");
  if (/^SX-\d{3}$/.test(base)) {
    return base;
  }
  if (/^XG-\d{3}$/.test(base)) {
    return `XG1-${base.slice(3)}`;
  }
  const match = base.match(/^XG(\d)-(\d{3})$/);
  if (match) {
    return `XG${match[1]}-${match[2]}`;
  }
  return null;
}
...
/** 全シリーズページから cardId → 画像 URL を構築。 */
export async function buildXgatherImageMap() {
  const map = new Map();
  for (const pageUrl of SERIES_PAGES) {
    const html = await fetchSeriesHtml(pageUrl);
    for (const match of html.matchAll(CARD_LINK_RE)) {
      const cardId = lightboxToCardId(match[1]);
      if (!cardId) continue;
      const imagePath = match[2];
      map.set(cardId, `${GRNRNGR_ORIGIN}${imagePath}`);
    }
  }
  return map;
}
...
async function main() {
  ...
  console.log("Scraping xgather series pages...");
  const imageMap = await buildXgatherImageMap();
  console.log(`Found ${imageMap.size} xgather/SX image URLs`);
  ...
  for (const catalogPath of CATALOGS) {
    ...
    for (const card of file.cards) {
      if (!XGATHER_CARD_ID_RE.test(card.id)) continue;
  ...
  if (missingInMap.length > 0) {
    console.warn(`Catalog xgather/SX cards missing from scrape: ${missingInMap.length}`);
    for (const id of missingInMap.slice(0, 10)) {
      console.warn(`  ${id}`);
    }
  }

  console.log(`\nDownloading ${tasks.length} xgather/SX images (dry-run=${dryRun})...\n`);
