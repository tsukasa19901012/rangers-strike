/**
 * Auto-generated pipeline test for RM-001 (蒸着)
 * Regenerate: npm run pipeline:card -- RM-001
 */
...
describe("RM-001 蒸着 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RM-001");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e892b8e79d80 — effect uses fallback_handler — engine test required", () => {});
});
