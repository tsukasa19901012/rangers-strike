/**
 * Auto-generated pipeline test for RS-186 (プロバイダーベース)
 * Regenerate: npm run pipeline:card -- RS-186
 */
...
describe("RS-186 プロバイダーベース (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-186");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e38397e383ad — effect uses fallback_handler — engine test required", () => {});
});
