/**
 * Auto-generated pipeline test for RS-607 (ファルコンサモナー)
 * Regenerate: npm run pipeline:card -- RS-607
 */
...
describe("RS-607 ファルコンサモナー (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-607");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e38395e382a1 — effect uses fallback_handler — engine test required", () => {});
});
