/**
 * Auto-generated pipeline test for RK-087 (ゼクトマイザー)
 * Regenerate: npm run pipeline:card -- RK-087
 */
...
describe("RK-087 ゼクトマイザー (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-087");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e382bce382af — effect uses fallback_handler — engine test required", () => {});
});
