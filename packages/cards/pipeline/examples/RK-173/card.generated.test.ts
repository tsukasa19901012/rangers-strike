/**
 * Auto-generated pipeline test for RK-173 (鬼の鎧)
 * Regenerate: npm run pipeline:card -- RK-173
 */
...
describe("RK-173 鬼の鎧 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-173");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e9acbce381ae — effect uses fallback_handler — engine test required", () => {});
});
