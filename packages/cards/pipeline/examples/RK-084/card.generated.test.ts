/**
 * Auto-generated pipeline test for RK-084 (トリックベント)
 * Regenerate: npm run pipeline:card -- RK-084
 */
...
describe("RK-084 トリックベント (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-084");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e38388e383aa — effect uses fallback_handler — engine test required", () => {});
});
