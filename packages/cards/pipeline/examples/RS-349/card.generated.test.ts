/**
 * Auto-generated pipeline test for RS-349 (スーパー戦隊アドレスブック)
 * Regenerate: npm run pipeline:card -- RS-349
 */
...
describe("RS-349 スーパー戦隊アドレスブック (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-349");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e382b9e383bc — effect uses fallback_handler — engine test required", () => {});
});
