/**
 * Auto-generated pipeline test for RS-523 (臨気)
 * Regenerate: npm run pipeline:card -- RS-523
 */
...
describe("RS-523 臨気 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-523");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e887a8e6b097 — effect uses fallback_handler — engine test required", () => {});
});
