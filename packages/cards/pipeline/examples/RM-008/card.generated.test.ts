/**
 * Auto-generated pipeline test for RM-008 (対バイオロン法)
 * Regenerate: npm run pipeline:card -- RM-008
 */
...
describe("RM-008 対バイオロン法 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RM-008");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e5afbee38390 — effect uses fallback_handler — engine test required", () => {});
});
