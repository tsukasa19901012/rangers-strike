/**
 * Auto-generated pipeline test for XG5-058 (ロイヤルストレートフラッシュ)
 * Regenerate: npm run pipeline:card -- XG5-058
 */
...
describe("XG5-058 ロイヤルストレートフラッシュ (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("XG5-058");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e383ade382a4 — effect uses fallback_handler — engine test required", () => {});
});
