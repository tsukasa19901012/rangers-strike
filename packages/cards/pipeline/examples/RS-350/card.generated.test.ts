/**
 * Auto-generated pipeline test for RS-350 (暗黒の鎧)
 * Regenerate: npm run pipeline:card -- RS-350
 */
...
describe("RS-350 暗黒の鎧 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-350");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e69a97e9bb92 — effect uses fallback_handler — engine test required", () => {});
});
