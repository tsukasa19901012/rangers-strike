/**
 * Auto-generated pipeline test for RS-249 (Vコマンダー)
 * Regenerate: npm run pipeline:card -- RS-249
 */
...
describe("RS-249 Vコマンダー (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-249");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_56e382b3e383 — effect uses fallback_handler — engine test required", () => {});
});
