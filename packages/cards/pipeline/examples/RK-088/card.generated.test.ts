/**
 * Auto-generated pipeline test for RK-088 (イマジンとの契約)
 * Regenerate: npm run pipeline:card -- RK-088
 */
...
describe("RK-088 イマジンとの契約 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-088");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e382a4e3839e — effect uses fallback_handler — engine test required", () => {});
});
