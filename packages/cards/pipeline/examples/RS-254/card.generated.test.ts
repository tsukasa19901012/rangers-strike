/**
 * Auto-generated pipeline test for RS-254 (バードニックウェーブ)
 * Regenerate: npm run pipeline:card -- RS-254
 */
...
describe("RS-254 バードニックウェーブ (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-254");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e38390e383bc — effect uses fallback_handler — engine test required", () => {});
});
