/**
 * Auto-generated pipeline test for XG1-050 (ライダーシンドローム)
 * Regenerate: npm run pipeline:card -- XG1-050
 */
...
describe("XG1-050 ライダーシンドローム (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("XG1-050");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e383a9e382a4 — effect uses fallback_handler — engine test required", () => {});
});
