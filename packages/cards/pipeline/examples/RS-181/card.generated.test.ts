/**
 * Auto-generated pipeline test for RS-181 (破邪百獣剣)
 * Regenerate: npm run pipeline:card -- RS-181
 */
...
describe("RS-181 破邪百獣剣 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-181");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e7a0b4e982aa — effect uses fallback_handler — engine test required", () => {});
});
