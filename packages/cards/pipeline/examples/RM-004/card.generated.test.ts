/**
 * Auto-generated pipeline test for RM-004 (巨獣特捜)
 * Regenerate: npm run pipeline:card -- RM-004
 */
...
describe("RM-004 巨獣特捜 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RM-004");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e5b7a8e78da3 — effect uses fallback_handler — engine test required", () => {});
});
