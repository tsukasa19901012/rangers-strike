/**
 * Auto-generated pipeline test for RK-013 (Jパワー)
 * Regenerate: npm run pipeline:card -- RK-013
 */
...
describe("RK-013 Jパワー (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-013");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_4ae38391e383 — effect uses fallback_handler — engine test required", () => {});
});
