/**
 * Auto-generated pipeline test for RS-185 (カーナビック)
 * Regenerate: npm run pipeline:card -- RS-185
 */
...
describe("RS-185 カーナビック (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RS-185");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e382abe383bc — effect uses fallback_handler — engine test required", () => {});
});
