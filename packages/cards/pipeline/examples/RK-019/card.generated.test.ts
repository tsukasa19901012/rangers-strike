/**
 * Auto-generated pipeline test for RK-019 (音撃)
 * Regenerate: npm run pipeline:card -- RK-019
 */
...
describe("RK-019 音撃 (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-019");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e99fb3e69283 — effect uses fallback_handler — engine test required", () => {});
});
