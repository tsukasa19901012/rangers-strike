/**
 * Auto-generated pipeline test for SX-008 (超・電王トリロジー)
 * Regenerate: npm run pipeline:card -- SX-008
 */
...
describe("SX-008 超・電王トリロジー (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("SX-008");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e8b685e383bb — effect uses fallback_handler — engine test required", () => {});
});
