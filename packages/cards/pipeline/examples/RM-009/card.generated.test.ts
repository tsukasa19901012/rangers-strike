/**
 * Auto-generated pipeline test for RM-009 (マルチパック)
 * Regenerate: npm run pipeline:card -- RM-009
 */
...
describe("RM-009 マルチパック (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RM-009");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e3839ee383ab — effect uses fallback_handler — engine test required", () => {});
});
