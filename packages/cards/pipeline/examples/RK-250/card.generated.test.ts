/**
 * Auto-generated pipeline test for RK-250 (サバイブ)
 * Regenerate: npm run pipeline:card -- RK-250
 */
...
describe("RK-250 サバイブ (pipeline)", () => {
  ...
  it("has expected card id and effects", () => {
    expect(doc.id).toBe("RK-250");
    expect(doc.effects?.length ?? 0).toBeGreaterThan(0);
  });

  it.skip("named_e382b5e38390 — effect uses fallback_handler — engine test required", () => {});
});
