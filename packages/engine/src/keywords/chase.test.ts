...
describe("chase keyword", () => {
  ...
  it("resolve_chase from battle moves rider to rush on the new vehicle", () => {
    const state = chaseState();
    state.players.player1.battle = [rider];
    state.players.player1.rush = [vehicle1, vehicle2];
    state.definitions["TST-V1"] = {
      id: "TST-V1",
      name: "Vehicle 1",
      type: "vehicle",
    ...
    expect(resolved.players.player1.battle.some((c) => c.instanceId === rider.instanceId)).toBe(
      false,
    );
    expect(resolved.players.player1.rush.some((c) => c.instanceId === rider.instanceId)).toBe(true);
    expect(
